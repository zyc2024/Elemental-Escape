...
/**
 * Primary view class for the game, abstracting the basic graphics calls.<p></p>
 * This version of GameCanvas only supports rectangular and polygonal
 * sprite drawing. It also supports a debug mode that draws polygonal outlines.
 * <p></p> Before any calls to draw, it is necessary to set up with begin()
 * and terminate the pass with end().<p></p>
 * Author: zyc2024 <br>
 * Adopted From: Walker M. White
 */
public class GameCanvas {
    ...
    public enum BlendState {
        /**
         * Alpha blending on, assuming the colors have pre-multiplied alpha
         * (DEFAULT)
         */
        ALPHA_BLEND,
        /**
        ...
        ADDITIVE,
        /**
         * Color values are draw on top of one another with no transparency
         * support
         */
        OPAQUE
    }
    ...
    /**
     * Track whether canvas is active (for error checking)
     */
    ...
    // END-REGION ================ CACHE OBJECTS ===============================


    /**
     * Creates a new GameCanvas determined by the application configuration.
     * <p> This constructor initializes all the necessary graphics objects.
     */
    public GameCanvas() {
    ...
    public void dispose() {
        if (active != DrawPass.INACTIVE) {
            Gdx.app.error("GameCanvas", "Cannot dispose while drawing active"
    ...
    public void setWidth(int width) {
        if (active != DrawPass.INACTIVE) {
            Gdx.app.error("GameCanvas", "Cannot alter property while drawing "
                    + "active", new IllegalStateException());
            return;
        }
        this.width = width;
        if (!isFullscreen()) {
    ...
    /**
     * Resets the SpriteBatch projection matrix when this canvas is resized.
     * <p>
     * This needs to be called whenever the application window is resized.
     */
    public void resize() {
     ...
     * state to composite with other textures.  Unlike the other setters, if
     * it is
     * perfectly safe to use this setter while  drawing is active (e.g.
     * in-between
     * a begin-end pair).
     *
     ...
     */
    public void clear() {
        // sets screen to all black
        Gdx.gl.glClearColor(0, 0, 0, 1.0f);
        Gdx.gl.glClear(GL20.GL_COLOR_BUFFER_BIT);
    }


    // BEGIN-REGION ================ STANDARD-DRAW =============================
     ...
     * <p>
     * Unless otherwise transformed by the global transform (@see begin
     * (Affine2)),
     * the texture will be unscaled.  The bottom left of the texture will be
     * positioned
     ...
     */
    public void draw(Texture image, Color tint, float x, float y, float width
            , float height) {
        if (active != DrawPass.STANDARD) {
            Gdx.app.error("GameCanvas", "Cannot draw without active begin()",
                    new IllegalStateException());
     ...
     * @param height The texture height
     */
    public void draw(Texture image, Color tint, float ox, float oy, float x,
     ...
     * any white into the given color.  Other colors will be similarly affected.
     * <p>
     * The transformations are BEFORE after the global transform (@see begin
     * (Affine2)).
     * As a result, the specified texture origin will be applied to all
     * transforms
     * (both the local and global).
     * <p>
     * The local transformations in this method are applied in the following
     * order:
     ...
     */
    public void draw(Texture image, Color tint, float ox, float oy, float x,
                     float y, float angle, float sx, float sy) {
        if (active != DrawPass.STANDARD) {
            Gdx.app.error("GameCanvas", "Cannot draw without active begin()",
                    new IllegalStateException());
            return;
     ...
     * <p>
     * A texture region is a single texture file that can hold one or more
     * textures.
    ...
    /**
     * Draws the tinted texture region (filmstrip) with the given
     * transformations
     * <p>
     * A texture region is a single texture file that can hold one or more
     * textures.
    ...
    public void draw(TextureRegion region, Color tint, float ox, float oy,
        ...
        spriteBatch.setColor(tint);
        spriteBatch.draw(region, region.getRegionWidth(),
                region.getRegionHeight(), local);
    }
     ...
     * <p>
     * A polygon region is a texture region with attached vertices so that it
     * draws a
    ...
    public void draw(PolygonRegion region, Color tint, float ox, float oy,
        ...
        spriteBatch.setColor(tint);
        spriteBatch.draw(region, x, y, ox, oy, bounds.getRegionWidth(),
                bounds.getRegionHeight(), sx, sy,
     ...
     * Nothing is flushed to the graphics card until the method end() is called.
     */
    public void beginDebug(Camera camera) {
    ...
    public void drawPhysics(PolygonShape shape, Color color, float x, float y) {
        if (active != DrawPass.DEBUG) {
            Gdx.app.error("GameCanvas", "Cannot draw without active " +
                    "beginDebug()", new IllegalStateException());
            return;
        }
     ...
     * Draws the outline of the given shape in the specified color
     * <p>
     * The position of the circle is ignored.  Only the radius is used. To
     * move the
    ...
    // END-REGION ================ DEBUG-DRAW ==================================


    // BEGIN-REGION ================ HELPERS ===================================
     ...
     * @param sy    The y-axis scaling factor
     */
    private void computeTransform(float ox, float oy, float x, float y,
                                  float angle, float sx, float sy) {
        local.setToTranslation(x, y);
